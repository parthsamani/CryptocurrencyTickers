...
async def scan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔍 Live Scanning {len(custom_symbols)} pairs ({user_settings['tf']})...")
    def do_scan():
        sigs, ovs = [], []
...
@app.route('/')
def home(): return f"LIVE Bot OK | TF {user_settings['tf']} | {len(custom_symbols)} pairs"
